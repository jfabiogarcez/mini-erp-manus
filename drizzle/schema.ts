...
/**
 * Tabela de conversas WhatsApp com clientes e motoristas
 */
export const conversasWhatsapp = mysqlTable("conversasWhatsapp", {
  id: int("id").autoincrement().primaryKey(),
  telefoneContato: varchar("telefoneContato", { length: 20 }).notNull(), // Número do cliente/motorista
  nomeContato: varchar("nomeContato", { length: 255 }),
  tipoContato: mysqlEnum("tipoContato", ["Cliente", "Motorista", "Fornecedor"]).notNull(),
  contatoId: int("contatoId"), // FK para membrosEquipe ou tabela de clientes
  ultimaMensagem: text("ultimaMensagem"),
  dataUltimaMensagem: timestamp("dataUltimaMensagem"),
  contextoConversas: text("contextoConversas"), // JSON com histórico resumido para memória
  ativo: int("ativo").default(1).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
export const mensagensWhatsapp = mysqlTable("mensagensWhatsapp", {
  id: int("id").autoincrement().primaryKey(),
  conversaId: int("conversaId").notNull().references(() => conversasWhatsapp.id, { onDelete: "cascade" }),
  telefoneOrigem: varchar("telefoneOrigem", { length: 20 }).notNull(),
  telefoneDestino: varchar("telefoneDestino", { length: 20 }).notNull(),
  conteudo: text("conteudo").notNull(),
  tipo: mysqlEnum("tipo", ["Entrada", "Saída"]).notNull(), // Mensagem recebida ou enviada
  statusEntrega: mysqlEnum("statusEntrega", ["Enviada", "Entregue", "Lida", "Erro"]).default("Enviada").notNull(),
  idTwilio: varchar("idTwilio", { length: 255 }), // ID da mensagem no Twilio
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
